...
module.exports = class OrigamiImageSetTools {
	 ...
	 */
	constructor(options) {
		this.options = Object.assign({}, OrigamiImageSetTools.defaults, options);
	...
	purgeFromImageService() {
		...
		return this.findUpdatedImages().then(images => Promise.all(images.map(image => {
			...
			const axiosConfig = {
				url: purgeUrl + schemeAndPath + '?source=oist',
				headers: {
					'ft-origami-api-key': this.options.imageServiceApiKey
				}
			};
			const axiosConfigExtension = {
				url: purgeUrl + schemeAndPathAndExtension + '?source=oist',
				headers: {
					'ft-origami-api-key': this.options.imageServiceApiKey
				}
			};
			...
			return Promise.all([
				...
				axios.get(axiosConfigExtension)
			])
				.then(() => {
				...
				.catch(error => {
					this.log.error(`✘ Could not schedule purge of "${schemeAndPath}" "${schemeAndPathAndExtension}" from "${this.options.imageServiceUrl}" using ${JSON.stringify(axiosConfig)}`);
					throw error;
				});
		})).then(() => {
