...
module.exports = class OrigamiImageSetTools {
	...
	publishToS3(bucket) {
		...
		return this.buildImageSetManifest().then(imageSetManifest => {
			return Promise.all(imageSetManifest.images.map(image => {
				...
				return Promise.all([
					s3.upload(s3Config).promise(),
					s3.upload(s3ConfigExt).promise()
				])
				.then(() => {
	...
	/**
	 * Schedules images in the image set to be purged from the Origami Image Service
	...
	purgeFromImageService() {
		...
		return this.buildImageSetManifest().then(imageSetManifest => {
			return Promise.all(imageSetManifest.images.map(image => {
				...
				.catch(error => {
					this.log.error(`✘ Could not schedule purge of "${schemeAndPath}" "${schemeAndPathAndExtension}" from "${this.options.imageServiceUrl}"`);
					throw error;
				});
			})).then(() => {
				// empty block used to ensure that the promise resolves with `undefined`
